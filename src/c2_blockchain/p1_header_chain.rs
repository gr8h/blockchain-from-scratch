...
fn build_valid_chain_length_5() -> Vec<Header> {
    ...
    for _ in 0..4 {
        let child_header = current_header.child();
...
fn build_an_invalid_chain() -> Vec<Header> {
    ...
    // Now we add a header with an invalid height.
    let mut invalid_header = current_header.child();
    invalid_header.height = 10;
    chain.push(invalid_header.clone());
